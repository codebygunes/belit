...
bool WasmLifter::parse(const std::vector<uint8_t>& rawBytecode) {
    ...
    while (offset < rawBytecode.size()) {
        ...
        if (sectionId == 2) {
            try {
                ...
                // STRICT HALT & PROVE: Parsing corruption detected in Section 2.
                // Do not fallback. Invalidate payload immediately.
                reset();
                return false;
            }
        }
        // Section 10: Code Parsing
            ...
            try {
                    ...
                    while (offset < functionEnd && offset < rawBytecode.size()) {
                        ...
                        switch (opcode) {
                            ...
                            default: 
                                ...
                                // An unmapped opcode immediately flags the payload as unverifiable.
                                reset();
                                return false;
                        }
                ...
                // STRICT HALT & PROVE: Corrupted Code section detected.
                reset();
                return false;
            }
        }
    ...
    // STRICT HALT & PROVE: Removed the dummy fallback block. 
    // If the CFG is empty, the payload is invalid. Do not generate fake instructions.
    return !cfg_.empty();
}
